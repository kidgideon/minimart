...
import styles from "./midBannerOne.module.css";
import { db, storage } from "../src/hooks/firebase";
import { doc, getDoc, updateDoc } from "firebase/firestore";
import { ref, uploadBytes, getDownloadURL } from "firebase/storage";
import { auth } from "../src/hooks/firebase";
import bannerFallback from "../src/images/banner.jpg";
import { motion, AnimatePresence } from "framer-motion";

const MidBannerOne = ({ showEdit = false }) => {
  ...
  useEffect(() => {
    const fetchBanner = async () => {
      try {
        ...
        const midBanner = bizDoc.data()?.layoutThemes?.midBannerOne || {};

        setBannerData(midBanner);
        setFormState({
          image: midBanner.image || "",
          mainText: midBanner.mainText || "",
          subText: midBanner.subText || "",
          display: midBanner.display !== false,
          darkOverlay: midBanner.darkOverlay !== false,
          align: midBanner.align || "left"
        });
        setImagePreview(midBanner.image || "");
      } catch (err) {
        console.error("Error loading mid banner:", err);
      } finally {
        setLoading(false);
      }
    };
    ...
    try {
      ...
      if (imageFile) {
        const storageRef = ref(
          storage,
          `midBanners/${businessId}/midBannerOne_${Date.now()}_${imageFile.name}`
        );
      ...
      await updateDoc(bannerRef, {
        "layoutThemes.midBannerOne": updatedBanner
      });
      ...
      console.error("Failed to update mid banner:", err);
    } finally {
      setSaving(false);
    }
  };
  ...
  // Only render if display is true
  if (bannerData && bannerData.display === false) {
    return null;
  }
  ...
  return (
    <div className={styles.midBannerInterface}>
      {loading ? (
        <div className={styles.loading}>
          <i className="fa-solid fa-spinner fa-spin"></i>
        </div>
      ) : (
        <div className={styles.bannerBg} style={{ backgroundImage: `url(${bannerImage})` }}>
          {bannerData?.darkOverlay && <div className={styles.darkOverlay}></div>}
          <div
            className={styles.texts}
            style={getTextAlignStyles(bannerData?.align || "left")}
          >
            {bannerData?.mainText && <h2>{bannerData.mainText}</h2>}
        ...
        {editMode && (
          ...
          >
            <div className={styles.modalContentModern}>
              <h2 className={styles.modalTitle}>Edit Inline Banner</h2>
              <div className={styles.imageEditSection}>
                <div className={styles.imageEditPreview} style={{ aspectRatio: "16/6", height: "200px", maxWidth: "100%" }}>
                  <img
              ...
              <div className={styles.formGrid}>
                ...
                <label className={styles.checkboxLabel}>
                  <input
                    ...
                    onChange={(e) =>
                      setFormState({ ...formState, display: e.target.checked })
                    }
                    disabled={saving}
                  />
